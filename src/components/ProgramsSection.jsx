...
export const ProgramsSection = ({ t, language = 'en' }) => {
    const localePrograms = Array.isArray(t.programsList) ? t.programsList : [];
    const fallbackPrograms = getProgramsForLocale(language);
    const programs = localePrograms.length > 0 ? localePrograms : fallbackPrograms;
    ...
    return (
        <div className="space-y-8">
            ...
            {isEmpty ? (
                <div className="rounded-3xl border border-dashed border-brand-navy/30 bg-white/40 p-6 text-center text-sm text-brand-navy/70">
                    {t.programsEmpty}
                </div>
            ) : (
